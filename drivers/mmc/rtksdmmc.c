...
static void rtk_sdmmc_init(struct rtk_sdmmc_host *rtk_host)
{
	if( rtk_host == NULL ) {
...
static int rtk_sdmmc_probe(struct udevice *dev)
{
	...
	host->crt_base = devfdt_get_addr_index(dev, 0);
    host->sdmmc_base = devfdt_get_addr_index(dev, 1);
    host->sysbrdg = devfdt_get_addr_index(dev, 2);
    host->emmc_base = devfdt_get_addr_index(dev, 3);
